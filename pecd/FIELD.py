...
class Field():
    ...
    def fieldRCPL(self, t, function_name, omega, E0, CEP0, spherical):
        ...
        if spherical == True:
            fieldvec = E0 * np.array( [  0.0 , 0.0,  np.cos( omega * t + CEP0 )  - 1j * np.sin( omega * t + CEP0 )  ] ) / np.sqrt(2.0)
        else:
            fieldvec = E0 * np.array( [  0.0 , 0.0,  np.cos( omega * t + CEP0 )  - 1j * np.sin( omega * t + CEP0 )  ] ) / np.sqrt(2.0)
        return fieldvec
    ...
    def fieldLCPL(self, t, function_name, omega, E0, CEP0, spherical):
        ...
        if spherical == True:
            fieldvec = E0 * np.array( [   - 1.0 * ( np.cos( omega * t + CEP0 ) + 1j * np.sin( omega * t + CEP0 ) ) , 0.0, 0.0] ) / np.sqrt(2.0)
        else:
            fieldvec = E0 * np.array( [   - 1.0 * ( np.cos( omega * t + CEP0 ) + 1j * np.sin( omega * t + CEP0 ) ) , 0.0, 0.0] ) / np.sqrt(2.0)
        return fieldvec
